...
static {
  ...
  private static X509TrustManager UNTRUSTED_TRUST_MANAGER=new X509TrustManager(){
...
;
  private static Logger logger=Logger.getLogger(MockWebServer.class.getName());
  private BlockingQueue<RecordedRequest> requestQueue=new LinkedBlockingQueue<>();
  private Set<Socket> openClientSockets=Collections.newSetFromMap(new ConcurrentHashMap<Socket,Boolean>());
  private Set<Http2Connection> openConnections=Collections.newSetFromMap(new ConcurrentHashMap<Http2Connection,Boolean>());
  ...
  @Override synchronized void before(){
    if (started)     return;
    try {
      start();
    }
 ...
 */
  ...
  @Override synchronized void after(){
    try {
      shutdown();
    }
 catch (    IOException e) {
  ...
  public void serveConnection(  final Socket raw){
 ...
 */
      public void createTunnel() throws IOException, InterruptedException {
 ...
 */
      public boolean processOneRequest(      Socket socket,      BufferedSource source,      BufferedSink sink) throws IOException, InterruptedException {
  ...
  public void processHandshakeFailure(  Socket raw) throws Exception {
  ...
  public void dispatchBookkeepingRequest(  int sequenceNumber,  Socket socket) throws InterruptedException {
 ...
 */
  public RecordedRequest readRequest(  Socket socket,  BufferedSource source,  BufferedSink sink,  int sequenceNumber) throws IOException {
    String request;
    try {
  ...
  public void handleWebSocketUpgrade(  Socket socket,  BufferedSource source,  BufferedSink sink,  RecordedRequest request,  MockResponse response) throws IOException {
...
;
  ...
  public void writeHttpResponse(  Socket socket,  BufferedSink sink,  MockResponse response) throws IOException {
  ...
  public void sleepIfDelayed(  long delayMs){
    if (delayMs != 0) {
      try {
 ...
 */
  public void throttledTransfer(  MockResponse policy,  Socket socket,  BufferedSource source,  BufferedSink sink,  long byteCount,  boolean isRequest) throws IOException {
  ...
  public void readEmptyLine(  BufferedSource source) throws IOException {
...
private static class TruncatingBuffer implements Sink {
    ...
    public TruncatingBuffer(    long bodyLimit){
...
private class Http2SocketHandler extends Http2Connection.Listener {
    ...
    public Http2SocketHandler(    Socket socket,    Protocol protocol){
      this.socket=socket;
    ...
    public RecordedRequest readRequest(    Http2Stream stream) throws IOException {
      ...
      if (!readBody && peek.getSocketPolicy() == EXPECT_CONTINUE) {
        stream.sendResponseHeaders(Collections.singletonList(new Header(Header.RESPONSE_STATUS,ByteString.encodeUtf8("100 Continue"))),true);
    ...
    public void writeResponse(    Http2Stream stream,    MockResponse response) throws IOException {
    ...
    public void pushPromises(    Http2Stream stream,    List<PushPromise> promises) throws IOException {
